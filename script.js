...
const todoApp = (function () {
    ...
    function toggleTodo(todo, array = todoContainer.classList.contains("filtered") ? filteredTodos : todos) {
        todo.toggleStatus();
...
const modalCategoriesDOM = (function () {

})();

const modalTodosDOM = (function () {
...
const helperFunctions = (function () {
    ...
    function renderTodos(array = todoContainer.classList.contains("filtered") ? todoApp.filteredTodos : todoApp.todos) {
    ...
    return { createDOMElement, createFormCategory, createForm, printExistentInputs, createButton, renderTodos, updatePage }
})();
...
function transformCatToInput(event, h3, li) {
    ...
    createFormElement(title, title, "visually-hidden", "input", "text");
    input.value = title;
...
newCatbtn.addEventListener("click", event => {
    openDialog(event);
    helperFunctions.createFormCategory();
});
...
function renderTodosPage(newTodo) {
    ...
    const priorityTag = li.appendChild(helperFunctions.createDOMElement("span", `${newTodo.priority}`));
